...
def detect(object_dto: Object, antennas_dto: list[Antenna], transmitters_dto: list[Transmitter],
    ...
    detected_position, regression_lines, all_circles, all_points = detect_by_method(method, phase_increment, antennas,
                                                                                    transmitters[0], object,
                                                                                    phase_error_coef)

    helpers = Helpers(regression_lines, all_circles, all_points)

    if detected_position is None:
        raise PositionIsNoneError

    return PositionDto(detected_position.x, detected_position.y), helpers


def create_heatmap(antennas_dto: list[Antenna], transmitters_dto: list[Transmitter], settings: SimulationSettings,
                   edge_length, resolution, update_progressbar, finish_progressbar):
    antennas: list[RxAntennaArray] = []
    for antenna_dto in antennas_dto:
        antennas.append(create_antenna(antenna_dto.dipole_number, antenna_dto.dipole_spread,
                                       settings.wavelength, map_position(antenna_dto.position)))
    ...
    create_heat_map(edge_length, resolution, method, method_name, antennas, transmitters[0], phase_error_coef,
                    update_progressbar, finish_progressbar)
