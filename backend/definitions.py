...
def plot_scenario(ax: plt.Axes, antennas: list, tx: TxDipole, obj_position: Position):
    ...
    for antenna in antennas:
        for dipole in antenna.dipoles:
            ...
            plot_line_between(ax, dipole.coordinates, obj_position, 'yellow')
...
def detect_object_using_antenna_set_variance(antennas: list, tx=None, obj_position=None, plot=False) -> Position:
    ...
    for antenna in antennas:
        dipole_distance_variance = calculate_figure_center_variance(
            [antenna.dipoles[0].coordinates, antenna.dipoles[-1].coordinates])
        ...
        for i in range(how_many_circles):
            ...
            for dipole in antenna.dipoles:
                ...
                circles.append([dipole.coordinates.x, dipole.coordinates.y, radius])
                if plot:
                    x =  radius * np.cos(angle) + dipole.coordinates.x
                    y =  radius * np.sin(angle) + dipole.coordinates.y
...
class RxAntennaArray:
    def __init__(self, dipoles: list):
        self.antenna_diameter = abs(dipoles[0].coordinates.x - dipoles[-1].coordinates.x)
        self.dipoles = dipoles
        self.antenna_center = calculate_figure_center([dipole.coordinates for dipole in dipoles])
